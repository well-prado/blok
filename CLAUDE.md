...
- Do NOT read an arm-local handle from a sibling arm or after the flow step, or read an `ephemeral: true` step's handle at all — there is no readable state slot. Return a value from the control-flow step, or write both arms to one `as:` key
