...
export default workflow({
	...
	trigger: {
		http: {
			method: "POST",
			path: "/webhook-handlers/github-push",
	...
	steps: [
		{
			...
			inputs: {
				level: "info",
				message:
					"js/`github-push: ${ctx.request.body.repo || '?'} ${ctx.request.body.ref || '?'} (${Array.isArray(ctx.request.body.commits) ? ctx.request.body.commits.length : 0} commits)`",
			},
			ephemeral: true,
		},
		{
