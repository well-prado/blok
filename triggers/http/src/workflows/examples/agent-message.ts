	...
	steps: [
		...
		},
		{
			...
			inputs: {
				...
				messages:
					"js/[{ role: 'system', content: 'You are a helpful agent with two tools: get_weather(city) returns current weather; calculate(expression) evaluates arithmetic. Use them when the user asks weather or math questions. Be concise.' }, ...(ctx.state['load-history']?.value || []), { role: 'user', content: ctx.request.body.message }]",
				tools: [
					{
						name: "get_weather",
		...
		},
		{
			...
			inputs: {
				...
				key: "js/`agent:${ctx.request.params.sessionId}:history`",
				value:
					"js/[...(ctx.state['load-history']?.value || []), { role: 'user', content: ctx.request.body.message }, { role: 'assistant', content: ctx.state.agent.fullText }]",
				ttlMs: 86400000,
			},
			ephemeral: true,
		},
