...
export default workflow({
	...
	trigger: {
		http: {
			method: "POST",
			path: "/webhook-handlers/stripe.invoice.paid",
	...
	steps: [
		{
			...
			inputs: {
				level: "info",
				message:
					"js/`stripe.invoice.paid: invoice=${ctx.request.body?.stripeEvent?.data?.object?.id ?? '?'} amount=${ctx.request.body?.stripeEvent?.data?.object?.amount_paid ?? '?'}`",
			},
			ephemeral: true,
		},
		{
