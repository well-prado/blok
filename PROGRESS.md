...
- ✅ **DX-1: HMR Trigger Integration (TriggerBase HMR lifecycle, all 8 triggers wired, ESM cache-bust, HmrDevConsole)** 🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉
...
- ✅ **CLI build fixed (added @blokjs/runner workspace dep, resolved type casting)** 🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉
...
6. ~~**Testing Coverage**: ~50% coverage, needs to reach 90%+~~ ✅ **RESOLVED!** (332 new tests across core/shared, workflow-helper, triggers/http, triggers/grpc, runtimes/python3 — all at 90%+!)
...
|------|-----------------|-----------|-------|----------------|
| @blokjs/api-call | ✅ Migrated | 2026-01-27 | 5/5 ✅ | 60% less boilerplate |
| @blokjs/if-else | ✅ Migrated | 2026-01-27 | 6/6 ✅ | 60% less boilerplate |
| @blokjs/react | ✅ Migrated | 2026-01-27 | 1/1 ✅ | 50% less boilerplate |
...
|---------|---------|--------|---------------|--------|-------|
| @blokjs/runner | 0.1.26 | ✅ Stable | 40% | Python lock-in | - |
| @blokjs/shared | 0.0.9 | ✅ Stable | 60% | None | - |
| @blokjs/helper | 0.1.5 | ✅ Stable | 50% | Limited types | - |
...
| Package | Version | Status | Test Coverage | Issues | Owner |
|---------|---------|--------|---------------|--------|-------|
| @blokjs/trigger-http | 0.0.34 | ✅ Production | 45% | None | - |
| @blokjs/trigger-grpc | 0.0.14 | ✅ Production | 40% | None | - |
| @blokjs/trigger-queue | 0.0.1 | ✅ Complete | 100% (9 tests) | None | - |
| @blokjs/trigger-pubsub | 0.0.1 | ✅ Complete | 100% (8 tests) | None | - |
| @blokjs/trigger-cron | 0.0.1 | ✅ Complete | 100% (6 tests) | None | - |
| @blokjs/trigger-webhook | 0.0.1 | ✅ Complete | 100% (18 tests) | None | - |
...
|---------|---------|--------|---------------|---------|-----------------|
| @blokjs/api-call | 0.1.29 | ✅ Production | 55% | Class | ❌ Not migrated |
| @blokjs/if-else | 0.0.30 | ✅ Production | 60% | Class | ❌ Not migrated |
| @blokjs/react | 0.0.17 | ✅ Production | 30% | Class | ❌ Not migrated |
...
protected nodeTypes(): NodeResolverTypes {
  return {
    module: {
      // Loads from node_modules (@blokjs/*)
...
- [ ] **Node Migration**
  - [ ] Migrate @blokjs/api-call
  - [ ] Migrate @blokjs/if-else
  - [ ] Migrate @blokjs/react
...
**DX-1: HMR Trigger Integration - COMPLETE:**
- ✅ **TriggerBase HMR Foundation** - Added `enableHotReload()`, `waitForInFlightRequests()`, overridable `onHmrNodeChange/onHmrWorkflowChange/onHmrTriggerChange` hooks
  - In-flight request counter (`inFlightRequests++` / `--` in `run()` try/finally) for zero-downtime reload
  - Production guard: disabled unless `BLOK_HMR=true` or `NODE_ENV=development`
  - Configurable via env vars: `WORKFLOWS_PATH`, `NODES_PATH`, `BLOK_HMR_VERBOSE`
- ✅ **ESM Cache-Bust** - Added `invalidateEsmModule()` to HotReloadManager (`path?t=timestamp`)
- ✅ **HmrDevConsole** - New utility for formatted dev-friendly HMR event logging with timestamps
- ✅ **HTTP Trigger** - `stop()` with graceful server shutdown, `onHmrNodeChange` override (cache invalidate + reload)
- ✅ **Webhook Trigger** - Lightweight workflow list refresh (no restart needed)
- ✅ **Cron Trigger** - Full stop/reload/restart cycle with in-flight drain
- ✅ **Queue Trigger** - Full stop/reload/restart cycle
- ✅ **PubSub Trigger** - Full stop/reload/restart cycle
- ✅ **Worker Trigger** - Full stop/reload/restart cycle with queue management
- ✅ **WebSocket Trigger** - Lightweight refresh (keeps client connections)
- ✅ **SSE Trigger** - Lightweight refresh (keeps client connections)
- ✅ **CLI dev command** - Passes `BLOK_HMR=true` and `NODE_ENV=development` to child processes
- ✅ **12 new HMR tests** (10 TriggerBaseHmr + 2 invalidateEsmModule) — all passing

**Pre-existing Build Error Fixes:**
- ✅ Fixed WasmRuntimeAdapter: removed unused `maxCacheAge` field
- ✅ Fixed FileWatcher: removed unused `basename` import, prefixed unused loop var
- ✅ Fixed SentryIntegration: `@ts-expect-error` for optional `@sentry/node` import, typed `span` param
- ✅ Fixed OpenAPIGenerator: prefixed unused loop var
- ✅ Fixed AuditLogger: narrowed `filter.since` via local variable (linter-safe)
- ✅ Fixed AuthMiddleware: proper runtime type guards for JWT payload fields (`exp`, `nbf`, `iat`, `sub`)
- ✅ **Runner now builds cleanly** (`pnpm run build` exits 0), **925 tests passing**

### 2026-01-28 - Cross-Package Test Coverage COMPLETED! (332 New Tests, 90%+ Coverage)

**QA-3: Test Coverage Initiative - 5 Packages Brought to 90%+ Coverage:**

**Phase 1: core/shared (0 → 90 tests):**
- ✅ Set up vitest infrastructure (vitest.config.ts, package.json scripts)
- ✅ `GlobalError.test.ts` - 12 tests (constructor, setCode, setJson, setStack, setName, hasJson, toString, toDict)
- ✅ `GlobalLogger.test.ts` - 6 tests (singleton, log/error/warn methods, child loggers)
- ✅ `Trigger.test.ts` - 2 tests (type export, schema validation)
- ✅ `Metrics.test.ts` - 8 tests (timer, counters, histograms, labels)
- ✅ `NodeBase.test.ts` - 22 tests (constructor, process, input mapping, output handling, error flows)
- ✅ `Mapper.test.ts` - 12 tests (mapInputs, resolveValue, dot notation, arrays, defaults)
- ✅ `CpuUsage.test.ts` - 7 tests (getCpuUsage, multi-core, idle, format)
- ✅ `MemoryUsage.test.ts` - 8 tests (getMemoryUsage, process, os, percentages)
- ✅ `Time.test.ts` - 5 tests (formatDuration, elapsed, timestamps)
- ✅ Additional edge case tests for full coverage

**Phase 2: core/workflow-helper (20 → 50 tests, +30 new):**
- ✅ `helperResponse.test.ts` - 5 tests (success/error response, data mapping)
- ✅ `trigger.test.ts` - 7 tests (all trigger types, validation, schema)
- ✅ `addIf.test.ts` - 6 tests (condition evaluation, truthy/falsy, nested)
- ✅ `addElse.test.ts` - 5 tests (else branch, default, chaining)
- ✅ `schemas.test.ts` - 12 tests (Zod schema validation for all types)
- ✅ Enhanced existing workflow.test.ts (+4), step.test.ts (+10), condition.test.ts (+3)

**Phase 3: triggers/http (0 → 36 tests):**
- ✅ Set up vitest infrastructure
- ✅ `MessageDecode.test.ts` - 12 tests (BASE64/STRING encoding, JSON/XML/TEXT/HTML types)
- ✅ `Util.test.ts` - 16 tests (handleDynamicRoute, nodeResolver, import mocking)
- ✅ `HttpTrigger.test.ts` - 8 tests (constructor, route registration, middleware, error handling)

**Phase 4: triggers/grpc (0 → 77 tests):**
- ✅ Set up vitest infrastructure (excluding src/gen/ protobuf from coverage)
- ✅ `MessageDecode.test.ts` - 31 tests (requestDecode, decodeType, responseEncode, responseErrorEncode, responseDecode, encodeType, mapContentType)
- ✅ `GrpcClient.test.ts` - 10 tests (constructor, transport types, call method, enum values)
- ✅ `NanoSDK.test.ts` - 14 tests (createClient, python3, nodejs, host resolution, nanosdk.json)
- ✅ `GrpcServer.test.ts` - 7 tests (constructor options, start, env vars)
- ✅ `GRpcTrigger.test.ts` - 12 tests (constructor, listen, processRequest, executeWorkflow with all node types)
- ✅ `Nodes.test.ts` - 3 tests (node map structure)
...
- ✅ `test_server.py` - 5 tests (ExecuteNode success/error/JSON, serve lifecycle)
- ✅ Fixed 3 existing broken tests (test_process_error, test_run_success, test_run_validation_error)

**Key Testing Patterns Used:**
- `vi.hoisted()` for mock variable declarations in vitest (avoids hoisting errors)
- Class-based mocks for modules instantiated with `new` (arrow functions can't be constructors)
- `vi.mock()` factory functions for full module mocking
- Python `unittest.mock.patch`, `AsyncMock`, `MagicMock` for async gRPC mocking

**New Files Created (31 test files):**
- `core/shared/vitest.config.ts` + 9 test files
- `core/workflow-helper/tests/` - 5 new test files + 3 enhanced
- `triggers/http/vitest.config.ts` + 3 test files
- `triggers/grpc/vitest.config.ts` + 6 test files
- `runtimes/python3/tests/` - 7 new test files + 2 fixed

**Total: 332 new tests across 5 packages, all passing!**
...
- ✅ Created `packages/vscode-extension/` - Full VS Code extension for Blok
  ...
  - **Hover Documentation**: Contextual docs on hover
    ...
    - Common node packages (@blokjs/api-call, @blokjs/if-else)
...
**DX-1: Hot Module Replacement (HMR) System - COMPLETE (Infrastructure + Trigger Integration):**
...
- ✅ Created `HotReloadManager.ts` - High-level HMR orchestrator
  ...
  - ESM cache-bust via `invalidateEsmModule()` (`path?t=timestamp`)
...
- ✅ Created `HmrDevConsole.ts` - Formatted dev-friendly HMR event logging with timestamps
- ✅ **TriggerBase HMR lifecycle** - `enableHotReload()`, `waitForInFlightRequests()`, overridable handlers
  - In-flight request counter in `run()` for zero-downtime reload
  - Production guard: `NODE_ENV=production` disables unless `BLOK_HMR=true`
- ✅ **All 8 triggers integrated** - HTTP, Webhook, Cron, Queue, PubSub, Worker, WebSocket, SSE
  - Two patterns: lightweight refresh (Webhook/WS/SSE) vs full stop/reload/restart (Cron/Queue/PubSub/Worker)
  - HTTP: graceful `stop()` with server close, `onHmrNodeChange` override
- ✅ **CLI dev command** - Passes `BLOK_HMR=true` + `NODE_ENV=development` to child processes
- ✅ **12 new HMR tests** (TriggerBaseHmr: 10, invalidateEsmModule: 2)
...
**Worker Trigger (`@blokjs/trigger-worker`) - COMPLETE:**
...
- ✅ Exported all monitoring classes and types from `@blokjs/runner`
...
- ✅ Created `@blokjs/trigger-queue` package
...
- ✅ Created `@blokjs/trigger-pubsub` package
...
- ✅ Created `@blokjs/trigger-cron` package
...
- ✅ Created `@blokjs/trigger-webhook` package
...
- ✅ Migrated [@blokjs/api-call](nodes/web/api-call@1.0.0/index.ts:1) to function-first (77 lines, 60% code reduction)
- ✅ Updated api-call tests to function-first pattern (5/5 tests passing)
- ✅ Migrated [@blokjs/if-else](nodes/control-flow/if-else@1.0.0/index.ts:1) to function-first (102 lines)
...
- ✅ Migrated @blokjs/react node to function-first pattern
...
- ✅ Exported from @blokjs/runner package
...
- ✅ Exported DockerRuntimeAdapter from `@blokjs/runner`
...
- ✅ Exported all runtime types from `@blokjs/runner`
...
Phase 2: Function-First Architecture  [████████████████████] 100% 🎉 (Phases 2A-2E Complete!)
...
```
@blokjs/runner                [████████████████████] 99%  (1051 tests) 🎉🎉🎉🎉
@blokjs/shared                [██████████████████░░] 92%  (90 tests) 🎉🎉 NEW!
@blokjs/helper                [██████████████████░░] 93%  (50 tests) 🎉🎉 NEW!
@blokjs/trigger-http          [██████████████████░░] 90%  (36 tests) 🎉🎉 NEW!
@blokjs/trigger-grpc          [██████████████████░░] 90%  (77 tests) 🎉🎉 NEW!
@blokjs/trigger-websocket     [████████████████████] 100% (50 tests) 🎉
@blokjs/trigger-sse           [████████████████████] 100% (49 tests) 🎉
@blokjs/trigger-worker        [████████████████████] 100% (32 tests) 🎉
@blokjs/api-call              [███████████░░░░░░░░░] 55%
@blokjs/if-else               [████████████░░░░░░░░] 60%
