...
**The expensive half is already built.** Each SDK emits a canonical JSON Schema from its typed input (`sdks/go/define_node.go:50-51` `reflectSchemaJSON[I]()`/`[O]()` via `invopop/jsonschema`; Python `pydantic` `model_json_schema`). The gRPC `ListNodes` RPC already returns `NodeDescriptor { name, description, input_schema_json, output_schema_json, tags }` (`proto/blok/runtime/v1/runtime.proto:272-275`) plus `sdk_version` (`:268`), and `Health` returns `registered_nodes` + `sdk_version` (`:288-289`). The schema-uniformity foundation Windmill spent years on already exists. **What's missing is a packaging envelope and a version-aware resolver — not a schema system.**
...
| Schema reflection done uniformly | `define_node.go:50-51`; pydantic; `NodeDescriptor.input_schema_json` (`runtime.proto:275`) | The manifest's `inputSchema` is free per impl |
...
- **Does this need to exist?** Yes — but *less than it looks*. The schema half is done. Do not rebuild it. The manifest's `inputSchema`/`outputSchema` are **copied verbatim** from `ListNodes` output, not re-derived.
