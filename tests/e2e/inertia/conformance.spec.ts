...
function appRequests(page: Page, from = 0): Request[] {
	return logFor(page)
		...
		.filter((request) => {
			...
			// CORS preflights are browser transport setup, not application visits.
...
/** Read the live server's order count without mutating it. */
async function ordersTotal(page: Page): Promise<number> {
	const response = await page.request.get(`${server}/orders`, { headers: { "X-Inertia": "true" } });
	expect(response.ok(), "the orders page could not report its current total").toBeTruthy();
	const body = (await response.json()) as { props?: { orders?: { total?: number } } };
	const total = body.props?.orders?.total;
	expect(total, "the orders page did not contain props.orders.total").toEqual(expect.any(Number));
...
test("30 — Precognition answers 204 for valid input without running the write", async ({ page }) => {
	await open(page, "/orders/create");
	const before = await ordersTotal(page);
	...
	await expect(page.getByTestId("page-heading")).toHaveText("New order");
	expect(await ordersTotal(page), "the Precognition probe performed the write").toBe(before);
});
