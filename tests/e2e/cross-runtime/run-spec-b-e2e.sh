...
# Go, Rust, C#, Java, PHP (via RoadRunner `rr`), Ruby (>= 3.1), Python3, Dart.
...
# --- Dart (gRPC 20008) ---
if command -v dart >/dev/null && [ -f "$ROOT/sdks/dart/pubspec.yaml" ]; then
  echo "--- booting Dart ---"
  (cd "$ROOT/sdks/dart" && dart pub get >/dev/null 2>&1)
  (cd "$ROOT/sdks/dart" && BLOK_TRANSPORT=grpc GRPC_PORT=20008 PORT=19008 dart run bin/serve.dart) >/tmp/blok-dart.log 2>&1 &
  PIDS+=($!); wait_port 20008 && echo "Dart gRPC up :20008"
fi
...
cd "$ROOT" && GO_GRPC_PORT=20001 RUST_GRPC_PORT=20002 JAVA_GRPC_PORT=20003 \
	CS_GRPC_PORT=20004 PHP_GRPC_PORT=20005 RUBY_GRPC_PORT=20006 PY_GRPC_PORT=20007 DART_GRPC_PORT=20008 \
