...
# Go, Rust, C#, Java, Kotlin, PHP (via RoadRunner `rr`), Ruby (>= 3.1), Python3, Swift.
...
if command -v java >/dev/null && [ -x "$ROOT/sdks/kotlin/gradlew" ]; then
  ...
  (cd "$ROOT/sdks/kotlin" && GRPC_PORT=20009 PORT=19009 BLOK_TRANSPORT=grpc ./build/install/blok-kotlin/bin/blok-kotlin) >/tmp/blok-kotlin.log 2>&1 &
  PIDS+=($!); wait_port 20009 && echo "Kotlin gRPC up :20009"
fi
...
cd "$ROOT" && GO_GRPC_PORT=20001 RUST_GRPC_PORT=20002 JAVA_GRPC_PORT=20003 \
	CS_GRPC_PORT=20004 KOTLIN_GRPC_PORT=20009 PHP_GRPC_PORT=20005 RUBY_GRPC_PORT=20006 PY_GRPC_PORT=20007 SWIFT_GRPC_PORT=20008 \
