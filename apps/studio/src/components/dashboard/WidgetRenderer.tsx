...
function StatCardWidget({
  ...
  const metricMap: Record<string, { value: string; color?: string }> = {
    ...
    errorRate: {
      value: formatPercent(
        metrics.totalRuns > 0 ? metrics.failedRuns / metrics.totalRuns : 0,
      ),
      color: metrics.failedRuns > 0 ? "text-red-400" : undefined,
    },
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full">
      <div className={cn("text-2xl font-bold font-mono", info.color || "text-zinc-100")}>
        {info.value}
      </div>
    </div>
  );
...
function ErrorRateWidget({ metrics }: { metrics: MetricsResponse }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-2">
      <div className={cn("text-3xl font-bold font-mono", isHigh ? "text-red-400" : "text-green-400")}>
        {pct}%
      </div>
