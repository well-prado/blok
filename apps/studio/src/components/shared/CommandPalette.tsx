...
import { Search, Workflow, Activity, ArrowRight, Command } from "lucide-react";
import { cn } from "@/lib/utils";
import { searchTraces, type SearchResponse } from "@/lib/api";
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh] pointer-events-none">
        <div className="w-full max-w-lg bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl overflow-hidden pointer-events-auto">
          {/* Search input */}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span
            className={cn("w-2 h-2 rounded-full flex-shrink-0", STATUS_DOT_COLORS[run.status])}
          />
        </div>
        <div className="text-[11px] text-zinc-500">
          {run.triggerSummary} · {formatDuration(run.durationMs)} · {formatRelativeTime(run.startedAt)}
          {run.error && (
            <span className="text-red-400"> · {run.error.message.slice(0, 60)}</span>
          )}
        </div>
      </div>
      {selected && <ArrowRight className="w-3 h-3 text-zinc-500 flex-shrink-0" />}
