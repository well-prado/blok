...
import type { NodeRun, RunDetail } from "@/types";
import { Link } from "@tanstack/react-router";
import { ArrowRight, CheckCircle2, Clock, MinusCircle, XCircle } from "lucide-react";
		...
		<div className="p-4 space-y-4">
			...
			<div className="grid grid-cols-4 gap-3">
				...
				<DiffStat
					...
					comparison={
						!runA.run.error && !runB.run.error ? "same" : runA.run.error && runB.run.error ? "same" : "different"
					}
				/>
			</div>
			...
			{/* Tab content */}
			{activeTab === "overview" && <OverviewTab runA={runA} runB={runB} nodeComparison={nodeComparison} />}
			...
			{activeTab === "outputs" && <OutputsTab nodeComparison={nodeComparison} />}
		</div>
	);
}
			...
			<div className="mt-2 space-y-1 text-xs text-zinc-500">
				...
				<div>
					...
					<span className="text-zinc-400">
						{run.run.completedNodes}/{run.run.nodeCount}
					</span>
				</div>
			</div>
		</div>
...
function OverviewTab({
	...
	const maxDuration = Math.max(...[...runA.nodes, ...runB.nodes].map((n) => n.durationMs || 0).filter((d) => d > 0), 1);

	return (
		<div className="space-y-4">
			<h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-600">Timeline Comparison</h3>
				...
				{selected ? (
					<div className="grid grid-cols-2 gap-4">
						<NodeCompareCard
							node={selected.nodeA}
							label="Run A"
							logs={runA.logs.filter((l) => l.nodeName === selected.name)}
						/>
						<NodeCompareCard
							node={selected.nodeB}
							label="Run B"
							logs={runB.logs.filter((l) => l.nodeName === selected.name)}
						/>
					</div>
				) : (
	...
	return (
		<div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-4 space-y-3">
			...
			<div className="space-y-1 text-xs text-zinc-500">
				<div>
					Duration: <span className="text-zinc-300 font-mono">{formatDuration(node.durationMs)}</span>
				</div>
				<div>
					Type: <span className="text-zinc-400">{node.nodeType}</span>
				</div>
				{node.runtimeKind && (
					<div>
						Runtime: <span className="text-zinc-400">{node.runtimeKind}</span>
					</div>
				)}
			</div>
			{node.error && (
...
function OutputsTab({
	...
	const nodesWithOutputDiff = nodeComparison.filter(
		({ nodeA, nodeB }) => nodeA?.outputs !== undefined || nodeB?.outputs !== undefined,
	);

	if (nodesWithOutputDiff.length === 0) {
		return (
			<div className="flex items-center justify-center h-48 text-zinc-600 text-sm">No node outputs to compare</div>
		);
	}
	...
	return (
		<div className="space-y-4">
			{nodesWithOutputDiff.map(({ name, nodeA, nodeB }) => {
				const outputsMatch = JSON.stringify(nodeA?.outputs) === JSON.stringify(nodeB?.outputs);
				...
				return (
					<div key={name} className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-4">
						<div className="flex items-center gap-2 mb-3">
							...
							{outputsMatch ? (
								<span className="text-[10px] px-1.5 py-0.5 rounded bg-green-500/10 text-green-400">Identical</span>
							) : (
								<span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400">Different</span>
							)}
						</div>
...
function compareDuration(a: number | undefined, b: number | undefined): "same" | "better" | "worse" | "different" {
