...
export function ExplainError({ runId, nodeId, compact }: ExplainErrorProps) {
	...
	if (state === "loading") {
		return (
			<div className={cn("flex items-center gap-2 text-xs text-purple-400", compact ? "px-2 py-1" : "px-2.5 py-1")}>
	...
	// state === "done"
	return (
		<div className={cn("rounded-md border border-purple-900/50 bg-purple-950/20", compact ? "mt-2" : "mt-3")}>
		...
		<>
			{parts.map((part, i) => {
				if (part.startsWith("**") && part.endsWith("**")) {
					return (
						// biome-ignore lint/suspicious/noArrayIndexKey: inline markdown parts have no stable ID
						<strong key={`strong-${i}`} className="font-semibold text-zinc-200">
							{part.slice(2, -2)}
						</strong>
					);
				}
				if (part.startsWith("`") && part.endsWith("`")) {
					return (
						// biome-ignore lint/suspicious/noArrayIndexKey: inline markdown parts have no stable ID
						<code key={`code-${i}`} className="bg-zinc-800 rounded px-1 py-0.5 text-[11px] font-mono text-purple-300">
							{part.slice(1, -1)}
						</code>
					);
				}
