...
wait: `for` and `until` are **literal-only** — no expression form of any kind resolves. `normalizeWaitStep` reads them at workflow LOAD time, before any request `ctx` exists, and `RunnerSteps.computeDeadline` only does `Number()`/`Date.parse()` on the stored value. The schema `.refine` requires EXACTLY ONE of `for`/`until` (both or neither is a hard error), so the editor validates that before save and offers no value picker. Note: `examples/v05-primitives/09-polling-with-backoff.json` used to encode a computed backoff here, which could never resolve; it now uses a literal interval and documents the limitation.
...
Branch condition editor: clicking a branch node opens a structural editor — [left operand] [comparator] [right operand] with the value picker on both operands and a `Negate (!)` box — over a live preview of the exact lowered `when` string. Lowering follows ADR 0004 (`specs/blok-vision/adr/0004-branch-when-lowering.md`): conditions are structural while authoring and lower to **raw** `ctx.*` JavaScript, never `js/`-prefixed (a `js/` `when` typechecks, passes WorkflowTestRunner, and 500s at runtime), with negation as `!(expr)` to match the shipped `not()`. The picker emits `js/`-prefixed exprs for step inputs, so the branch editor strips that prefix; `lowerCondition` strips it defensively on both operands too. `parseCondition` is deliberately conservative — it round-trips only bare `ctx…` expressions and single-comparator forms, and returns `null` (falling back to a raw-JS textarea with the original text intact) for `&&`/`||`/ternaries/calls, a bare `undefined` operand (real case in `v05-travel-booking.json`), and single-quoted JS strings (real case in `v07-ws-echo.json`) — re-lowering those would silently corrupt a working condition. Arms are not edited here (they are canvas-edited); the drawer shows a read-only `then: N steps · else: N steps` summary. Live acceptance: opened `decide-on-failure` in `v05-nested-control-flow`, whose existing condition parsed into structural form, flipped `===` to `!==`, and saved — the file got `ctx.state.item.required !== true` with both arms preserved.
...
Skip / Stop node-header toggles shipped 2026-08-06: hover icon buttons flip `active: false` (runner skips the step, flow continues) / `stop: true` (runner halts BEFORE the step) on the raw step via `toggleStepSkip`/`toggleStepStop`. The engine honored both flags all along (`RunnerSteps.runSteps`); the normalizer and `WorkflowTestRunner` already thread them, so the slice is write-path + rendering only (skipped card dims + dashes, stop card gets an amber dashed outline, its immediate outgoing edges dash). A skipped step emits a transient `NODE_SKIPPED` stream event but no persisted `NodeRun`; a stopped-at step records nothing — surfacing both in the run view is an open follow-up. Live acceptance: skipped `fill-email` + stop on `assert-dashboard-url` in `e2e-login` → run completed executing exactly launch/open-login/fill-password/sign-in.

Next visual/interaction candidates are ranked at the end of the research notes (drawer-open camera choreography first; edge splice, drag-from-socket, and skip/stop toggles shipped 2026-08-06).
