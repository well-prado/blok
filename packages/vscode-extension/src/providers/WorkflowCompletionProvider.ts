...
export class WorkflowCompletionProvider implements vscode.CompletionItemProvider {
	...
	private createNodeCompletions(): vscode.CompletionItem[] {
		const nodes = [
			{ label: "@blokjs/api-call", docs: "HTTP API call node - makes requests to external services" },
			{ label: "@blokjs/if-else", docs: "Conditional branching node - evaluates conditions for routing" },
			{ label: "@blokjs/react", docs: "React SSR node - server-side rendering" },
		];
