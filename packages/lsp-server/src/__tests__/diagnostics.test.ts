...
describe("WorkflowDiagnostics (LSP)", () => {
	...
	describe("required fields", () => {
		...
		it("should report empty name", () => {
			const diagnostics = validateWorkflow(JSON.stringify({
				name: "",
				version: "1.0.0",
	...
	describe("node reference validation", () => {
		...
		it("should not warn about nodes used in conditions", () => {
			...
			const unusedDiag = diagnostics.find((d) => d.message.includes("nested-step") && d.message.includes("not referenced"));
	...
	describe("valid workflows", () => {
		it("should produce no diagnostics for a valid HTTP workflow", () => {
			const diagnostics = validateWorkflow(JSON.stringify({
				...
				steps: [
					{ name: "fetch-users", node: "@nanoservice-ts/api-call", type: "module" },
				],
				nodes: {
					"fetch-users": {
		...
		it("should produce no diagnostics for a valid queue workflow", () => {
			const diagnostics = validateWorkflow(JSON.stringify({
				...
				steps: [
					{ name: "process", node: "./nodes/process-event", type: "local" },
				],
				nodes: { process: {} },
			}));
			expect(diagnostics).toHaveLength(0);
