...
export async function setupRuntime(
	...
	switch (runtime.kind) {
		...
		case "java":
			...
			break;
		case "kotlin":
			startCmdOverride = await setupKotlin(blokctlRuntimeDir, spinner);
			break;
		case "csharp":
	...
	} else if (runtime.kind === "kotlin") {
		generateKotlinNodeRegistry(projectDir);
	} else if (runtime.kind === "csharp") {
		generateCSharpNodeRegistry(projectDir);
	} else if (runtime.kind === "swift") {
...
/**
 * Generate the Kotlin user-node registration shim. Kotlin is compiled, so
 * project nodes are copied into the SDK source set and registered through a
 * generated object. The scan deliberately accepts only classes implementing
 * NodeHandler; the node name remains the directory name used by the workflow.
 */
export function generateKotlinNodeRegistry(projectDir: string): string {
	const kotlinSdkDir = path.join(projectDir, ".blok", "runtimes", "kotlin");
	const nodesSrcDir = path.join(projectDir, "runtimes", "kotlin", "nodes");
	const usernodesDir = path.join(kotlinSdkDir, "src", "main", "kotlin", "usernodes");
	const registryFile = path.join(kotlinSdkDir, "src", "main", "kotlin", "com", "blok", "kotlin", "UserNodeRegistry.kt");
	fsExtra.removeSync(usernodesDir);
	...
	if (fsExtra.existsSync(nodesSrcDir)) {
		for (const entry of fsExtra.readdirSync(nodesSrcDir, { withFileTypes: true })) {
			if (!entry.isDirectory()) continue;
			const srcDir = path.join(nodesSrcDir, entry.name);
			const ktFiles = collectFilesRecursive(srcDir, ".kt");
			let className: string | undefined;
			let packageName = "usernodes";
			for (const file of ktFiles) {
				const source = fsExtra.readFileSync(file, "utf8");
				const match = source.match(/(?:class|object)\s+(\w+)\s*(?:\([^)]*\))?\s*:\s*NodeHandler\b/);
				if (!match) continue;
				className = match[1];
				packageName = source.match(/^\s*package\s+([\w.]+)/m)?.[1] ?? packageName;
				break;
			}
			if (!className) {
				console.warn(`[blokctl] skipping Kotlin node '${entry.name}': no class implementing NodeHandler found`);
				continue;
			}
			if (seenClasses.has(`${packageName}.${className}`)) {
				console.warn(`[blokctl] skipping Kotlin node '${entry.name}': duplicate handler ${packageName}.${className}`);
				continue;
			}
			seenClasses.add(`${packageName}.${className}`);
			const destDir = path.join(usernodesDir, entry.name);
			fsExtra.ensureDirSync(destDir);
			for (const file of ktFiles) fsExtra.copySync(file, path.join(destDir, path.basename(file)));
			registrations.push(`    registry.register(${packageName}.${className}())`);
		}
	}

	const content = `// Code generated by blokctl. DO NOT EDIT.\npackage com.blok.kotlin\n\n/** Registers nodes scaffolded under runtimes/kotlin/nodes. */\nobject UserNodeRegistry {\n    fun registerUserNodes(registry: NodeRegistry) {\n${registrations.join("\n")}\n    }\n}\n`;
	fsExtra.ensureDirSync(path.dirname(registryFile));
	fsExtra.writeFileSync(registryFile, content);
	return registryFile;
}
...
/**
 * Generate the Dart user-node registration shim. Dart AOT and `dart run` do
 * not support importing arbitrary files discovered at runtime, so user node
 * sources are copied into the SDK package and a deterministic import shim is
 * generated before the sidecar starts.
 */
export function generateDartNodeRegistry(projectDir: string): string {
	const dartSdkDir = path.join(projectDir, ".blok", "runtimes", "dart");
	const nodesSrcDir = path.join(projectDir, "runtimes", "dart", "nodes");
	const usernodesDir = path.join(dartSdkDir, "lib", "src", "user_nodes");
	const registryFile = path.join(dartSdkDir, "lib", "src", "generated_user_nodes.dart");
	fsExtra.removeSync(usernodesDir);
	fsExtra.ensureDirSync(usernodesDir);
	...
	if (fsExtra.existsSync(nodesSrcDir)) {
		for (const entry of fsExtra.readdirSync(nodesSrcDir, { withFileTypes: true })) {
			if (!entry.isDirectory()) continue;
			const source = path.join(nodesSrcDir, entry.name, "node.dart");
			if (!fsExtra.existsSync(source)) continue;
			const identifier = `userNode${imports.length}`;
			const destination = path.join(usernodesDir, `${identifier}.dart`);
			fsExtra.copySync(source, destination);
			imports.push(`import 'user_nodes/${identifier}.dart' as ${identifier};`);
			calls.push(`\tregistry.register(${identifier}.registerNode());`);
		}
	}
...
/** Kotlin uses its checked-in Gradle wrapper for reproducible JVM builds. */
async function setupKotlin(sdkDir: string, spinner: SpinnerHandler): Promise<string> {
	spinner.message("Building Kotlin project with the pinned Gradle wrapper...");
	await exec("./gradlew installDist --no-daemon", { cwd: sdkDir, timeout: 600000 });
	spinner.message("Kotlin project built.");
	return "./build/install/blok-kotlin/bin/blok-kotlin";
}
...
/** Swift: resolve the SwiftPM graph and build the Linux sidecar. */
async function setupSwift(sdkDir: string, spinner: SpinnerHandler): Promise<void> {
...
/**
 * Generate the Swift user-node registration shim.
 *
 * Swift is compiled, so user nodes are copied into the SDK target and a
 * generated registry calls their `register(_:)` entry point on every `dev`
 * boot. Node names remain workflow-facing strings; source filenames are not
 * used as identifiers.
 */
export function generateSwiftNodeRegistry(projectDir: string): string {
	const swiftSdkDir = path.join(projectDir, ".blok", "runtimes", "swift");
	const nodesSrcDir = path.join(projectDir, "runtimes", "swift", "nodes");
	const userNodesDir = path.join(swiftSdkDir, "Sources", "BlokSwiftRuntime", "UserNodes");
	const registryFile = path.join(swiftSdkDir, "Sources", "BlokSwiftRuntime", "GeneratedUserNodeRegistry.swift");
	fsExtra.removeSync(userNodesDir);
	fsExtra.ensureDirSync(userNodesDir);
	...
	if (fsExtra.existsSync(nodesSrcDir)) {
		for (const entry of fsExtra.readdirSync(nodesSrcDir, { withFileTypes: true })) {
			if (!entry.isDirectory()) continue;
			const srcDir = path.join(nodesSrcDir, entry.name);
			const swiftFiles = fsExtra.readdirSync(srcDir).filter((file) => file.endsWith(".swift"));
			if (swiftFiles.length === 0) continue;
			let namespace: string | undefined;
			for (const file of swiftFiles) {
				const source = fsExtra.readFileSync(path.join(srcDir, file), "utf8");
				if (!/static\s+func\s+register\s*\(/.test(source)) continue;
				namespace = source.match(/(?:public\s+)?enum\s+([A-Za-z_][A-Za-z0-9_]*)/)?.[1];
				if (namespace) break;
			}
			if (!namespace) {
				console.warn(`[blokctl] skipping Swift node '${entry.name}': no enum with static register(_:) found`);
				continue;
			}
			const module = entry.name.replace(/[^A-Za-z0-9_]/g, "_").replace(/^[0-9]/, "_$&");
			const destDir = path.join(userNodesDir, module);
			fsExtra.ensureDirSync(destDir);
			for (const file of swiftFiles) fsExtra.copySync(path.join(srcDir, file), path.join(destDir, file));
			registrations.push(`\t${namespace}.register(registry)`);
		}
	}

	const calls = registrations.join("\n");
	const content = `// Code generated by blokctl. DO NOT EDIT.\npublic enum GeneratedUserNodeRegistry {\n\tpublic static func registerAll(_ registry: NodeRegistry) {\n${calls}\n\t}\n}\n`;
	fsExtra.ensureDirSync(path.dirname(registryFile));
	fsExtra.writeFileSync(registryFile, content);
	return registryFile;
}
...
async function setupDart(sdkDir: string, spinner: SpinnerHandler): Promise<void> {
	spinner.message("Installing Dart dependencies...");
	await exec("dart pub get", { cwd: sdkDir, timeout: 120000 });
	spinner.message("Dart dependencies installed.");
}
