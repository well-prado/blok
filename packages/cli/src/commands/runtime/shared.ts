...
export function assertSidecarKind(kind: string): void {
	if (NON_SIDECAR_KINDS.has(kind)) {
		throw new RuntimeCommandError(
			`"${kind}" runs in-process and is always available — there's no sidecar to add or remove. Sidecar runtimes: go, rust, java, csharp, php, ruby, python3, swift, dart.`,
		);
	}
	if (!getRuntimeDefinition(kind)) {
		throw new RuntimeCommandError(
			`Unknown runtime "${kind}". Supported: go, rust, java, csharp, php, ruby, python3, swift, dart.`,
		);
	}
}
