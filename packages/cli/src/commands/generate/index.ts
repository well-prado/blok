		...
		withErrorBoundary(async (options: OptionValues) => {
			await trackCommandExecution({
				...
				execution: async () => {
					...
					if (!options.name) {
						throw new Error(
							!options.update && !options.prompt
								? "Both --name and --prompt options are required."
					...
					if (!options.update) {
						...
						if (options.code) {
							...
							if (!fs.existsSync(options.codeFile)) {
								throw new Error(`File not found: ${options.codeFile}`);
							}
			...
			await trackCommandExecution({
				...
				execution: async () => {
					...
					if (!options.prompt && !options.update) {
						throw new Error("The --prompt option is required for new workflows.");
					}
		...
		withErrorBoundary(async (options: OptionValues) => {
			await trackCommandExecution({
				...
				execution: async () => {
					...
					if (!options.prompt && !options.update) {
						throw new Error("The --prompt option is required for new triggers.");
					}
		...
		withErrorBoundary(async (options: OptionValues) => {
			await trackCommandExecution({
				...
				execution: async () => {
					...
					if (!options.prompt && !options.update) {
						throw new Error("The --prompt option is required for new runtimes.");
					}
