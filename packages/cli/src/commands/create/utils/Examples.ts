...
- **Nodes can be written in 10 runtimes.** TypeScript runs in-process; the other 9 (\`go\`, \`rust\`, \`java\`, \`kotlin\`, \`csharp\`, \`php\`, \`ruby\`, \`python3\`, \`swift\`) run as gRPC sidecar processes. A step routes to a sidecar via \`type: "runtime.<lang>"\`.

The 9 trigger types: \`http\`, \`worker\`, \`cron\`, \`pubsub\`, \`sse\`, \`websocket\`, \`webhook\`, \`mcp\`, \`grpc\`.
The 10 runtimes: \`typescript\` (in-process), \`go\`, \`rust\`, \`java\`, \`kotlin\`, \`csharp\`, \`php\`, \`ruby\`, \`python3\`, \`swift\`.
...
- gRPC dispatch port = legacy HTTP port + 1000. **Dispatch ports:** go \`10001\`, rust \`10002\`, java \`10003\`, csharp \`10004\`, php \`10005\`, ruby \`10006\`, python3 \`10007\`, swift \`10008\`, kotlin \`10009\`. (Readiness/health HTTP ports are the legacy \`9001\`–\`9009\`; the CLI readiness check is a **TCP connect to the gRPC port**, not \`GET /health\`.)
