 ...
 */
export const BLOKJS_DEP_RANGE = "^2.5.0";
...
export async function createProject(opts: OptionValues, version: string, currentPath = false, localRepoPath?: string) {
	...
	try {
		...
		for (const triggerKind of selectedTriggers) {
			...
			if (triggerKind === "pubsub" || triggerKind === "queue" || triggerKind === "worker") {
				...
				if (triggerKind === "sse" || triggerKind === "websocket") {
					...
					// Serverless deployments reuse the same trigger source through the
					// provider-neutral Web Fetch adapter. Keep this file in generated
					// projects so `--serverless` remains self-contained.
					if (fsExtra.existsSync(`${triggerSrcDir}/serverless.ts`)) {
						fsExtra.copySync(`${triggerSrcDir}/serverless.ts`, `${triggerDestDir}/serverless.ts`);
					}
		...
		// #1083 — opt-in Vercel/serverless deployment files. Do this before the
		...
		if (opts.serverless) {
			...
			if (!/^BLOK_SERVERLESS=/m.test(currentEnv)) {
				fsExtra.appendFileSync(envLocal, "\n# Vercel/serverless runtime\nBLOK_SERVERLESS=1\n");
			}
		}
		...
		// The command error boundary (#899) prints the message once and sets
		// the exit code.
...
/**
 * Write the minimal Vercel Function entrypoint and routing manifest for an
 * HTTP project. Existing files are treated as user-owned and cause a clear
 * error instead of being overwritten.
 */
export function scaffoldServerlessDeployment(dirPath: string): void {
	const apiDir = path.join(dirPath, "api");
	const apiEntry = path.join(apiDir, "index.ts");
	const vercelConfig = path.join(dirPath, "vercel.json");
	const existing = [apiEntry, vercelConfig].filter((file) => fsExtra.existsSync(file));
	if (existing.length > 0) {
		throw new Error(
			`Serverless scaffold would overwrite existing file${existing.length > 1 ? "s" : ""}: ${existing.join(", ")}. Move them or remove --serverless and run again.`,
		);
	}
	fsExtra.ensureDirSync(apiDir);
	fsExtra.writeFileSync(
		apiEntry,
		`/** Vercel Function entrypoint generated by blokctl. */
