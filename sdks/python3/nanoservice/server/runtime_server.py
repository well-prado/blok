...
from nanoservice.types.execution_result import ExecutionResult

logger = logging.getLogger("nanoservice")


class _RequestHandler(BaseHTTPRequestHandler):
    """HTTP request handler for the nanoservice runtime."""
...
class RuntimeServer:
    """Nanoservice HTTP server that handles execute and health requests.
    ...
    def start(self) -> None:
        ...
        logger.info(
            "Nanoservice runtime v%s starting on %s",
            self.config.version,
            self.config.address,
        )
