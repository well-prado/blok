...
```json
{
  ...
  "nodes": {
    "fan-out-notifications": {
      "inputs": {
        "branches": [
          { "stepName": "send-email", "node": "@blokjs/api-call" },
          { "stepName": "send-sms", "node": "@blokjs/api-call" },
          { "stepName": "send-push", "node": "@blokjs/api-call" }
        ],
        "waitForAll": true
      }
    }
...
- Use the `@blokjs/if-else` node to detect failures and route to compensating steps.
