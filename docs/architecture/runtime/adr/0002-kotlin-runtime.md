...
`runtime.kotlin` is a distinct canonical runtime kind on gRPC port `10011`
(readiness HTTP port `9011`). The Kotlin SDK is built under `sdks/kotlin` with
...
- `packages/cli/tests/commands/runtime/runtime.test.ts` covers Kotlin runtime
  detection/configuration, port `10011`, and `runtime add kotlin` vocabulary.
