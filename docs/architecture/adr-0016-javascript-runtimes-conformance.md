...
| Engine floors | `packages/js-runtime-worker/src/host.ts`; `packages/cli/src/services/runtime-detector.ts`; `.github/workflows/ci.yml` | Pass: Node.js 20, Bun 1.1, Deno **2.7.5**. The Deno floor was bisected against the worker's own integration suite (2.7.4 fails, 2.7.5 passes) because Deno's Node-compat HTTP/2 server did not complete a gRPC connection before then — the port binds and no call ever returns. CI pins exactly that version so the declared floor is the floor CI proves. |
