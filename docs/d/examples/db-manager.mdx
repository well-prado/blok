...
The `database-ui` node is responsible for serving the HTML, CSS, and JavaScript for the UI. When a GET request is made to the root path (`/db-manager`), the `branch` step routes the request to this node.
...
2. The `branch` step evaluates the condition:
   ```javascript
   eq($.req.method.toLowerCase(), "get") && $.req.params.function === undefined
   ```
...
1. **GET /db-manager/tables**: Lists available database tables
   ```javascript
   eq($.req.method.toLowerCase(), "get") && eq($.req.params.function, "tables")
   ```

2. **POST /db-manager/execute-prompt**: Generates and executes an SQL query
   ```javascript
   eq($.req.method.toLowerCase(), "post") && eq($.req.params.function, "execute-prompt")
   ```

### Benefits of This Approach

Using `branch` steps for UI rendering in Blok  offers several advantages:
...
4. **Conditional Logic**: Complex routing can be implemented with expressions using `eq()` and other primitives
...
2. **Use Branch Steps**: Configure conditions using `eq()` and other branch primitives based on request properties
...
5. **Define API Endpoints**: Add nested branch steps for API endpoints in the same workflow
...
export default defineNode({
  name: "mapper",
  description: "Maps data from one format to another",
  
  input: z.object({
    mapper: z.record(z.unknown()),
  }),
