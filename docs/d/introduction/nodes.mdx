...
A node is a `defineNode()` call from `@blokjs/core`: a name, a Zod `input`
schema, a Zod `output` schema, and an `execute` function. Everything else is
optional.
...
`input` is validated and typed before `execute` runs; the return value is
validated against `output`. Import the node into a workflow and hand the object
to `step()` — never reference it by string:

```typescript
import { workflow, step, http } from "@blokjs/core";
import greet from "./nodes/greet";

export default workflow("greet", { version: "1.0.0", trigger: http.get("/greet") }, (req) => {
  step("greet", greet, { name: req.query.name });
});
```

The rest of this page covers the CLI scaffold, validation, error handling, and
the built-in nodes.
...
In TypeScript, Nodes are defined using the `defineNode()` helper from `@blokjs/core`. The CLI scaffolds this for you:
...
The Context (`ctx`) object is a crucial part of the Blok framework. It's passed to each node's `execute` function and provides:
...
Blok ships several built-in nodes as standalone packages. Import the node and
hand the object straight to `step()` — there's no registry file to maintain:

```typescript
import { workflow, step, http } from "@blokjs/core";
import apiCall from "@blokjs/api-call";

export default workflow("countries", { version: "1.0.0", trigger: http.get("/countries") }, (req) => {
  step("fetch", apiCall, { url: "https://countriesnow.space/api/v0.1/countries", method: "GET" });
});
```
...
2. **Branching**:
   - Conditional flow is a first-class primitive: `branch(id, condition, { then, else })`
   - Use the typed comparators (`eq`, `ne`, `gt`, `gte`, `lt`, `lte`, `not`) for the condition
