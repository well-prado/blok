---
title: "Overview"
description: "Build modular, observable, testable backends with Blok — workflows of focused nodes that run across runtimes."
icon: "book-open"
---
...
* HTTP, SSE, WebSocket, Webhook, and MCP (all mount on one shared HTTP port)
* Worker (queue jobs), Cron (schedules), PubSub (cloud messages), and gRPC

Triggers are defined declaratively and allow the same logic to be accessed from multiple interfaces. See the [Triggers reference](/d/triggers/index) for the full list and per-kind config.
