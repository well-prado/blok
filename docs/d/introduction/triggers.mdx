---
title: "Triggers"
description: "How workflows start — HTTP requests, queue messages, schedules, and other events."
icon: "zap"
---
...
*   **MCP**: Exposes a workflow as a [Model Context Protocol](/d/triggers/mcp) tool or resource, so AI clients (Claude Code, Cursor, Claude Desktop) can discover and call it. The tool's input schema is generated from the workflow's Zod `input`.
...
## Reference pages per trigger kind

For the full config-field reference, `ctx` mapping, and worked patterns per trigger:

<Columns cols={4}>
  <Card title="HTTP" icon="globe" href="/d/triggers/http">REST endpoints + file-based routing</Card>
  <Card title="SSE" icon="cable" href="/d/triggers/sse">Server-Sent Events streaming</Card>
  <Card title="WebSocket" icon="plug" href="/d/triggers/websocket">Bidirectional WS connections</Card>
  <Card title="Webhook" icon="webhook" href="/d/triggers/webhook">Signed inbound w/ HMAC verify</Card>
  <Card title="MCP" icon="robot" href="/d/triggers/mcp">Workflows as AI tools (Claude, Cursor)</Card>
  <Card title="Worker" icon="factory" href="/d/triggers/worker">Background jobs from a queue</Card>
  <Card title="Cron" icon="clock" href="/d/triggers/cron">Time-based schedules</Card>
  <Card title="Pub/Sub" icon="share-2" href="/d/triggers/pubsub">Cloud pub/sub (GCP, AWS, Azure)</Card>
  <Card title="gRPC" icon="network" href="/d/triggers/grpc">Contract-based RPC</Card>
</Columns>

This concludes the core concepts in the Introduction. You can now explore [Practical Examples](../examples/index.mdx) or dive deeper into [Fundamentals](/d/fundamentals/blok-concept).
