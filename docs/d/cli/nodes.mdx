---
title: Node
icon: "puzzle-piece"
---

**Nodes** are the core building blocks of your **Blok** applications, encapsulating specific pieces of logic. The `blokctl` CLI provides commands to help you create and manage these Nodes efficiently, primarily through the `create node` command.

## `create node`

This command is used to scaffold a new Node within your **Blok** project.

> <sub>**Purpose**: To generate the boilerplate files for a new Node, including its definition (`node.json`) and implementation file (e.g., `index.ts`), placing them in the appropriate directory within your project (usually `nodes/`).</sub>

**Usage**:

```bash
# This command creates a new node and automatically generates a new directory for it.
npx blokctl@latest create node
```
```bash
# Use this command to create a new node inside the current (empty) directory.
npx blokctl@latest create node .
```

### 1. Interactive Prompts & Options

You will be prompted to provide the following information:

1. **Node Name**  
    Enter a unique name for the node (e.g., `fetch`).

2. **Select Runtime**  
    Choose between:
    - **TypeScript (recommended)** *(Default)*
    - **Python3** *(Alpha version, MacOS & Linux only)*  
    - **Swift** *(Swift 6.1+, Linux production sidecar)*
    *(For this guide, select **TypeScript**.)*

3. **Select Blok Type**  
    Choose between:
    - **Module** *(For npm-published nodes – not needed for this guide)*
    - **Class** *(Default, for local nodes)*

4. **Select Template**  
    Choose between:
    - **Class (recommended)** *(Default)*
    - UI - EJS - REACTJS - TailwindCSS *(not required for this guide)*

### 2. Navigate to the New Node Directory:

Once the node is created, navigate to its directory:

```bash
cd src/nodes/fetch
```

### 3. Update the Node Definition

Open the `index.ts` file inside the node directory. For TypeScript, update the node name in the `defineNode()` call to match your node (`fetch` in this example):

```typescript
// Original
export default defineNode({
  name: "node",
  ...
});

// Updated
export default defineNode({
  name: "fetch",
  ...
});
```

### 4. Registration is automatic

There's no registry to edit. A TypeScript node placed at `src/nodes/<name>/index.ts` is **auto-discovered** and registered by its `defineNode` `name` — the same string you reference from a workflow. `name` is the canonical reference, and a duplicate name throws at load.

```ts src/nodes/fetch/index.ts
import { defineNode } from "@blokjs/core";
import { z } from "zod";

export default defineNode({
  name: "fetch",   // ← this IS the registration; reference it as step("...", fetch, …)
  input: z.object({ url: z.string() }),
  output: z.object({ body: z.unknown() }),
  async execute(_ctx, input) {
    const res = await fetch(input.url);
    return { body: await res.json() };
  },
});
```

Published node packages don't need registration either — import them directly where a workflow uses them (`import apiCall from "@blokjs/api-call"`).

For Python nodes, they are auto-discovered from `runtimes/python3/nodes/` via the `BLOK_NODES_DIR` environment variable — no manual registration is needed.

### 5. Validate the Node

To verify that your new node has been registered correctly, run the following command from the root of your project:
```bash
npm run dev
```
If everything works as expected, the node is ready to be used in a workflow. Simply create a workflow and add your node to it.

## Summary

- ✅ **Created a custom node** using the CLI.  
- ✅ **Configured the node class name** correctly.  
- ✅ **Registered the node** inside `Nodes.ts`.  
- ✅ **Validated the project setup**.

Your node is now ready to be used in workflows! 🎉
