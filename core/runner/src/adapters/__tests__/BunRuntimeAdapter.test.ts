...
describe("BunRuntimeAdapter", () => {
	...
	describe("non-Bun host", () => {
		it("should detect non-Bun runtime", () => {
			...
			expect(BunRuntimeAdapter.isAvailable()).toBe(false);
		});

		it("fails closed without spawning a per-step process", async () => {
			(adapter as any).isBunRuntime = false;
			const result = await adapter.execute({} as RunnerNode, createMockContext());
			expect(result.success).toBe(false);
