...
describe.skipIf(!JAVA_AVAILABLE)("TS runner ↔ Java SDK over gRPC (E2E)", () => {
	...
	it("Java BlokError.dependency() flows through gRPC with all structured fields preserved", async () => {
		// Master plan §17 end-to-end check: a Java handler that throws
	...
	it("Java BlokError.rateLimit() propagates retry hints", async () => {
	...
	it("Java BlokError.validation() maps to 400 with non-retryable + structured issues", async () => {
