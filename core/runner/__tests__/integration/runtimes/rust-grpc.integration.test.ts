...
describe.skipIf(!RUST_BINARY_AVAILABLE)("TS runner ↔ Rust SDK over gRPC (E2E)", () => {
	...
	it("Rust BlokError::dependency() flows through gRPC with all structured fields preserved", async () => {
		// Master plan §17 end-to-end check: a Rust handler that returns
		// `BlokError::dependency()...build()` with a remediation, retry hints,
	...
	it("Rust BlokError::rate_limit() propagates retry hints", async () => {
	...
	it("Rust BlokError::validation() maps to 400 with non-retryable + structured issues", async () => {
