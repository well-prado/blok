...
describe.skipIf(!RUBY_AVAILABLE)("TS runner ↔ Ruby SDK over gRPC (E2E)", () => {
	...
	it("Ruby BlokError.dependency flows through gRPC with all structured fields preserved", async () => {
		// Master plan §17 end-to-end check: a Ruby handler that raises
		// `BlokError.dependency(...)` with a remediation, retry hints,
	...
	it("Ruby BlokError.rate_limit propagates retry hints", async () => {
	...
	it("Ruby BlokError.validation maps to 400 with non-retryable + structured issues", async () => {
