...
describe.skipIf(!CSHARP_AVAILABLE)("TS runner ↔ C# SDK over gRPC (E2E)", () => {
	...
	it("C# BlokError.Dependency() flows through gRPC with all structured fields preserved", async () => {
		// Master plan §17 end-to-end check: a C# handler that throws
	...
	it("C# BlokError.RateLimit() propagates retry hints", async () => {
	...
	it("C# BlokError.Validation() maps to 400 with non-retryable + structured issues", async () => {
