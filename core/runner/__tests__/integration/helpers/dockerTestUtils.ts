 ...
 */
export async function buildDockerImage(tag: string, context: string, dockerfile = "Dockerfile"): Promise<void> {
	console.log(`🐳 Building Docker image: ${tag}`);

	try {
		const { stdout, stderr } = await execAsync(`docker build -t ${tag} -f ${dockerfile} ${context}`, {
			cwd: process.cwd(),
		});
...
export async function startContainer(
	...
	const { stdout } = await execAsync(`docker run -d --name ${name} ${portMappings} ${envVars} ${image}`);
 ...
 */
export async function waitForHealthy(nameOrId: string, timeoutMs = 30000): Promise<void> {
	const startTime = Date.now();

	while (Date.now() - startTime < timeoutMs) {
		try {
			const { stdout } = await execAsync(`docker inspect --format='{{.State.Health.Status}}' ${nameOrId}`);
			...
			try {
				const { stdout } = await execAsync(`docker inspect --format='{{.State.Running}}' ${nameOrId}`);
 ...
 */
export async function waitForPort(host: string, port: number, timeoutMs = 30000): Promise<void> {
	const startTime = Date.now();
