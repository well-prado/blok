...
export class PerformanceMeasure {
	 ...
	 */
	async measure<T>(fn: () => Promise<T>, metadata?: Record<string, any>): Promise<T> {
	 ...
	 */
	async measureMultiple<T>(fn: () => Promise<T>, iterations: number, metadata?: Record<string, any>): Promise<T[]> {
		const results: T[] = [];
 ...
 */
export async function quickMeasure<T>(name: string, fn: () => Promise<T>, iterations = 10): Promise<PerformanceStats> {
 ...
 */
export function createPerformanceTable(results: Record<string, PerformanceStats>): string {
	const headers = ["Metric", "Mean (ms)", "P95 (ms)", "P99 (ms)", "Samples"];
	const rows: string[][] = [];

	for (const [name, stats] of Object.entries(results)) {
		rows.push([name, stats.mean.toFixed(2), stats.p95.toFixed(2), stats.p99.toFixed(2), stats.samples.toString()]);
	}
	...
	const lines = [formatRow(headers), separator, ...rows.map((row) => formatRow(row))];
