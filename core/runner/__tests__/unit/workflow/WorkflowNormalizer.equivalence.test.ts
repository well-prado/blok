...
describe("WorkflowNormalizer equivalence: three DSLs, one resolved IR", () => {
	...
	it("rejects a half-migrated v2 envelope (schemaVersion '2' still carrying nodes{})", () => {
		// Narrow #391 guard: a `{name,node}` step alongside `{id,use}` steps is NOT
		// a conflict (the normalizer harmonizes them — that freedom is what the
		// reverted over-strict guard broke). The genuine "partially migrated" smell
		// is an explicit v2 envelope that still drags the legacy nodes{} map.
		expect(() =>
			normalizeWorkflow({
				schemaVersion: "2",
				name: "mixed",
				...
				steps: [{ id: "seed", use: seedNode.name, inputs: { trace: "seed" } }],
				nodes: { seed: { inputs: { trace: "seed" } } },
			}),
		).toThrow(/schemaVersion: "2".*nodes\{\}/);
	});

	it("ACCEPTS the {name,node} + {id,use} mix the reverted guard wrongly rejected", () => {
		expect(() =>
			normalizeWorkflow({
				name: "mixed-ok",
				version: "1.0.0",
				...
				nodes: { legacy: { inputs: { trace: "legacy" } } },
			}),
		).not.toThrow();
	});
